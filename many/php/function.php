...
function PassTime($time){
	...
	return (strtotime($enddate)-strtotime($startdate))/86400; //天数
...
function XDINGsr($time,$name){
	$today = time();
	$bstamp = strtotime($time);
	$rev = ceil(($bstamp-$today)/(24*3600));
	if($rev == '0'){
		return "今天".$name."生日哦，祝".$name."生日快乐！";
	}else{
		return "距离".$name."的生日还有".$rev."天";
	}
}
